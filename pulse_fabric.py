...
def scout_agent(req):
    ...
    elif "telefon" in text or "tckn" in text or "lokasyon" in text:
...
def conductor_agent(inspector, guardian):
    if inspector["maturity_score"] < 70:
...
with m1:
    st.markdown('<div class="card"><h3>26 MD</h3><p>Current manual effort</p></div>', unsafe_allow_html=True)

with m2:
    st.markdown('<div class="card"><h3>40–70%</h3><p>Target reduction in request maturation</p></div>', unsafe_allow_html=True)

with m3:
    st.markdown('<div class="card"><h3>95%+</h3><p>Target request accuracy</p></div>', unsafe_allow_html=True)
...
with left:
    ...
    description = st.text_area("Description", request["description"], height=140)
...
with right:
    st.header("Digital Teammates")

    st.markdown("""
<div class="agent-card"><b>🧭 The Scout</b><br>Finds domain, owner and request type.</div>
<div class="agent-card"><b>🔍 The Inspector</b><br>Checks maturity and missing fields.</div>
...
<div class="agent-card"><b>🛡️ The Guardian</b><br>Checks guardrails, KVKK and approvals.</div>
...
""", unsafe_allow_html=True)
    ...
    with c2:
        card_class = "success-card" if inspector["status"] == "PASS" else "warning-card"
        st.markdown(f"""
...
""", unsafe_allow_html=True)
    ...
    guardian_class = "risk-card" if guardian["decision"] == "HUMAN APPROVAL REQUIRED" else "success-card"

    st.markdown(f"""
<div class="{guardian_class}">
...
""", unsafe_allow_html=True)
    ...
    with h1:
        if st.button("Approve"):
            st.success("Human approval captured. Request can proceed.")

    with h2:
        if st.button("Send Back"):
            st.warning("Request sent back for clarification.")
